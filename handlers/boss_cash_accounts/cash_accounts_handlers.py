...
@router.message(StateFilter(NavigationStates.waiting_change_account_currency))
async def process_cash_account_currency_input(
    message: types.Message,
    session: AsyncSession,
    state: FSMContext,
    db_user: User,
    ...
    """
    Обработка ввода нового значения currency для CashAccount
    """
    # Проверяем права
    if not db_user.is_boss:
        await message.answer("⛔ Только для боссов!")
    ...
    if new_currency not in allowed_currencies:
        cancel_keyboard = get_cancel_change_account_keyboard(cash_account_id)
        await message.answer(
            "⚠️ Доступно <u>только</u> <b>RUB</b> или <b>USD</b>",
            reply_markup=cancel_keyboard,
        )
        await message.delete()
        return
    ...
    try:
        updated_cash_account = await cash_account_service.update_account_currency(account_id=cash_account_id, new_currency=new_currency)
